...
UI.Pager = function (selector, options) {
    ...
    function bindData() {
        ...
        options.ajax.data.pageIndex = options.currentPage;
...
/*
   根据制定的容器，遍历其容器下的所有Form元素对象，将值进行收集最后组合为JSON对象
   id:      容器名
   array:   需要追加的其他JSON数据(可选)
   return:  返回一个生成的JSON对象
*/
UI.GenJsonDataFromContainer = function(id, array) {
    var str = "";
    $("#" + id).find("input").each(function() {
        ...
        if (item.attr("id")) {
            ...
            case "hidden":
            case "select":
                str += item.attr("id") + ":\"" + item.val() + "\",";
                break;
            case "checkbox":
                str += item.attr("id") + ":\"" + (item.attr("checked") ? 1 : 0) + "\",";
                break;
            case "radio":
                str += item.attr("id") + ":\"" + (item.attr("checked") ? 1 : 0) + "\",";
                break;
    ...
    return eval("({" + str + "})"); //encodeURIComponent("{" + str + "}");
};
